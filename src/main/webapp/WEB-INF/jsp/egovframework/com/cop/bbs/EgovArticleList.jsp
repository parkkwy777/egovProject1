...
<script src="/egovframework-all-in-one/webjars/jquery/3.5.1/dist/jquery.min.js"></script>
<script type="text/javascript">
		...
		if($(".allCheck").is(":checked")){
			$(".check").prop("checked",true);
				...
				for(var i=0; i<$(".check").length; i++){
					delArr.push($(".check").eq(i).val());
				}
			console.log(delArr);
		}else{
			$(".check").prop("checked",false);
			delArr=[];
			console.log("초기화 arr="+delArr);
		}
	});
	$(".check").click(function(){
		if($(this).is(":checked")){
			...
			for(let i =0; i<delArr.length; i++){
				...
				if(delArr[i]==$(this).val()){
					console.log("현재 value"+$(this).val());
					delArr.splice(i,1);
					console.log("삭제후"+delArr);
				}
			}
...
<div class="board">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
		<ul>
			...
			<li>
				...
				<span class="btn_b" style="background-color:red; color:white;"><a class="delete"><spring:message code="button.create" /> <spring:message code="input.button" />"><spring:message code="button.delete" /></a></span>
			</li>
		</ul>
	</div>
	...
	<thead>
	<tr>
		<th><input type="checkbox" name="nttId" class="allCheck"/></th>
	...
	<tr>
		<td><input type="checkbox" name="nttId" class="check" value="${noticeInfo.nttId}"/></td>
	...
	<tr>
		<td><input type="checkbox" name="nttId" class="check" value="${resultInfo.nttId}"/>nttid값: ${resultInfo.nttId}</td>
